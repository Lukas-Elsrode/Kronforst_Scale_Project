...
def print_axis_components(pca, features=DEF_FEATURES):
...
def family_analysis(wt_data, c_map, num_features=3):
    ...
    for fam in fams:
        if len(fam['scale_color'].unique()) == 1:
            print('FAMILY HAS ONLY ONE SCALE COLOR IN DATA: ',
                  fam['family'].unique()[0])
            pass
        else:
            print('FAMILY: ', fam['family'].unique()[0])
