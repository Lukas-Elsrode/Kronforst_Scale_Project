...
def save_data(data, file_name):
    ...
    """
    os.chdir('../data')
    data[0:100].to_csv(file_name, sep='\t', encoding='utf-8')
    return


def main(color_classification='closest', mutant_analysis=False, colors=None, N=2):
...
if __name__ == "__main__":
    ...
    print(txt)
    color.reindex_hierarchy(WT_DATA, with_color=True)
    c_map = color.fill_cmap(WT_DATA, on_index=False)
    viz_data.Load_Features(WT_DATA, c_map)
    #main(color_classification='closest', mutant_analysis=False, N=3)
